...
func (store *dbStore) CreateBird(bird *Bird) error {
	...
	// The first undescore means that we don't care about what's returned from this insert query.
	// We just want to know if it was inserted correctly, and the error will be populated if it wasn't
...
// The store variable is a package level variable that will be available for use throughout our application code
var store Store
