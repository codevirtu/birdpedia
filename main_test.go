...
func TestHandler(t *testing.T) {
	...
	// The first argument is the method, the second argument is the route, and the third is the request body, which we don't have in the case.
	...
	// We use Go's httptest library to create an http recorder. This recorder. This recorder will act as the target of our http request
	...
	if actual != expected {
		t.Errorf("hanlder returned unexpected body: got %v want %v", actual, expected)
	}
}
...
func TestRouter(t *testing.T) {
	...
	// We want our reponse to match the one defined in our handler.
	// If it does happen to be "Hello world!", the it confirms, that the route is correct
...
func TestRouterForNonExistentRoute(t *testing.T) {
	...
	// request to a route we know we didn't define, like the `PUT /hello` route.
	...
	// The code to test the body is also mostly the same, expect this time, we need an empty body
	defer resp.Body.Close()
	b, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		t.Fatal(err)
...
func TestStaticFileServer(t *testing.T) {
	...
	// Instead, we test that the content-type headerd is "text/html; charset=utf-8"
	// so that we know that an html file has been served
	contextType := resp.Header.Get("Content-Type")
	expectedContentType := "text/html; charset=utf-8"

	if expectedContentType != contextType {
		t.Errorf("Wrong content type, expected %s, got %s", expectedContentType, contextType)
	}
}
